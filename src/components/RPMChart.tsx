...
import { fetchTelemetryRPM, fetchSessionDrivers, fetchLapTimes, SessionDriver, RPMDataPoint } from '@/lib/api';
...
const RPMChart: React.FC<RPMChartProps> = ({
  ...
  // Fetch RPM data
  const { data: rpmData, isLoading: isLoadingRPM, error } = useQuery<RPMDataPoint[]>({
    queryKey: ['rpmTrace', year, event, session, selectedDriver, selectedLap],
    queryFn: () => fetchTelemetryRPM(year, event, session, selectedDriver, selectedLap),
  ...
  const handleDownload = async () => {
    if (!chartRef.current || isLoading || !rpmData || rpmData.length === 0) {
      return;
    }
    ...
    try {
      ...
      const filename = `${event.toLowerCase().replace(/\s+/g, '-')}_${selectedDriver}_${selectedLap === 'fastest' ? 'fastest' : `lap${selectedLap}`}_rpm`;
  ...
  const renderContent = () => {
    ...
    if (!shouldLoadChart) {
      return (
        <div className="w-full h-[280px] flex flex-col items-center justify-center bg-gray-900/50 rounded-lg gap-4">
          <p className="text-gray-400">Select a driver and click load to view RPM data</p>
          <Button 
    ...
    if (error || !rpmData) {
      return (
        <div className="w-full h-[280px] bg-gray-900/80 border border-red-500/30 rounded-lg flex flex-col items-center justify-center text-red-400">
          ...
          <p className="font-semibold">Error loading RPM data</p>
    ...
    if (rpmData.length === 0) {
      return (
        <div className="w-full h-[260px] bg-gray-900/80 border border-gray-700/50 rounded-lg flex items-center justify-center text-gray-500">
          No RPM telemetry data found for {selectedDriver} lap {selectedLap}.
        </div>
      );
    }
    ...
    return (
      <ResponsiveContainer width="100%" height={280} className="export-chart-container">
        <LineChart data={rpmData} margin={{ top: 0, right: 10, left: -15, bottom: 5 }} className="chart-main-container">
          ...
          <Line 
            ...
            strokeWidth={2} 
            dot={false} 
            activeDot={{ r: 4, strokeWidth: 1, stroke: 'rgba(255,255,255,0.5)', fill: "#fb923c" }} 
            name={selectedDriver} 
            connectNulls={true} 
          />
        </LineChart>
  ...
  return (
    ...
    >
      ...
      <CardContent className="pt-0">
        {renderContent()}
        {shouldLoadChart && !isLoading && rpmData && rpmData.length > 0 && (
